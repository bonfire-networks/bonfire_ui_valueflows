defmodule Bonfire.UI.ValueFlows.CreateEconomicEventLive.LiveHandler do
  ...
  def handle_event(
        "autocomplete" = action,
        %{"autocomplete_resource_conforms_to" => search} = attrs,
        socket
      )
  ...
  def handle_event(
        "autocomplete" = action,
        %{"autocomplete_resource_inventoried_as" => search} = attrs,
        socket
      )
      when is_binary(search) and byte_size(search) > 0 do
  ...
  def handle_event(
        "autocomplete" = action,
        %{"autocomplete_at_location" => search} = attrs,
        socket
      )
      when is_binary(search) and byte_size(search) > 0 do
  ...
  def handle_event("autocomplete" = action, %{"autocomplete_receiver" => search} = attrs, socket)
      when is_binary(search) and byte_size(search) > 0 do
